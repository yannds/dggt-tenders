  ...
  <div class="w-full">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
      <h2 class="text-2xl font-bold flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-rose-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0-1.657 1.343-3 3-3s3 1.343 3 3v2a3 3 0 01-3 3H9a3 3 0 01-3-3v-2a3 3 0 013-3c1.657 0 3 1.343 3 3z" /></svg>
        <span>Sécurité — Logs système (ISO 27001)</span>
      </h2>
    </div>
    <fieldset class="p-4 mb-6 bg-rose-50/60 shadow-sm rounded-2xl border-0">
      <div class="flex flex-wrap gap-3 mt-0 items-center justify-center">
        <input v-model="filters.keyword" type="text" placeholder="Recherche générale..." class="input input-xs w-44 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition placeholder-gray-400" />
        <select v-model="filters.user" class="select select-xs w-36 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition hover:bg-rose-50/80">
          <option value="">Utilisateur</option>
          <option v-for="u in uniqueUsers" :key="u" :value="u">{{ u }}</option>
        </select>
        <select v-model="filters.type" class="select select-xs w-36 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition hover:bg-rose-50/80">
          <option value="">Type</option>
          <option v-for="t in uniqueTypes" :key="t" :value="t">{{ t }}</option>
        </select>
        <select v-model="filters.ip" class="select select-xs w-32 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition hover:bg-rose-50/80">
          <option value="">Adresse IP</option>
          <option v-for="ip in uniqueIPs" :key="ip" :value="ip">{{ ip }}</option>
        </select>
        <select v-model="filters.status" class="select select-xs w-28 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition hover:bg-rose-50/80">
          <option value="">Statut</option>
          <option v-for="s in uniqueStatus" :key="s" :value="s">{{ s }}</option>
        </select>
        <select v-model="filters.session" class="select select-xs w-28 rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition hover:bg-rose-50/80">
          <option value="">Session</option>
          <option v-for="sess in uniqueSessions" :key="sess" :value="sess">{{ sess }}</option>
        </select>
        <button class="btn btn-xs btn-circle rounded-full shadow bg-white/80 border-0 text-rose-700 hover:bg-rose-100 transition" @click="resetFilters" title="Réinitialiser les filtres" :disabled="!hasActiveFilters">
    ...
    <div class="overflow-x-auto rounded-2xl shadow-lg bg-white mb-6">
      <table class="table w-full user-datagrid">
        <thead>
          <tr>
            <th class="bg-rose-50 px-2 w-8 text-center">
              <input type="checkbox" v-model="allSelected" />
            </th>
            <th class="bg-rose-50 px-4 min-w-[120px]">Date</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">Utilisateur</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">Type</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">Objet</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">IP</th>
            <th class="bg-rose-50 px-4 min-w-[100px]">Statut</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">Session</th>
            <th class="bg-rose-50 px-4 min-w-[120px]">ISO 27001</th>
            <th class="bg-rose-50 px-4 min-w-[80px] text-center">Détail</th>
          </tr>
        </thead>
        <tbody>
          <template v-for="log in paginatedLogs" :key="log.id">
            <tr class="align-middle hover:bg-rose-50/40 transition cursor-pointer" @click="selectLog(log)">
              <td class="px-2 text-center">
                <input type="checkbox" v-model="selectedIds" :value="log.id" @click.stop />
              </td>
              <td class="px-4">{{ log.date }}</td>
              <td class="px-4 font-semibold text-gray-900">{{ log.user }}</td>
              <td class="px-4">{{ log.type }}</td>
              <td class="px-4">{{ log.object }}</td>
              <td class="px-4 text-xs text-gray-500">{{ log.ip }}</td>
              <td class="px-4"><span :class="log.status === 'Succès' ? 'badge badge-success' : 'badge badge-error'">{{ log.status }}</span></td>
          ...
          <tr v-if="paginatedLogs.length === 0">
            <td colspan="10" class="text-center text-gray-400 py-8">Aucun log à afficher</td>
          </tr>
        </tbody>
      </table>
    </div>
    <fieldset class="p-4 mt-4 bg-gray-50/60 shadow-sm rounded-2xl border-0">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between w-full gap-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <select v-model.number="pageSize" class="select select-xs rounded-2xl shadow bg-white/80 focus:ring-2 focus:ring-rose-300 focus:bg-white border-0 transition">
    ...
    <div v-if="selectedLog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-2xl border border-rose-100 relative">
        ...
        <h3 class="text-xl font-bold mb-4 text-rose-700">Détail du log sécurité</h3>
        <div class="mb-2"><b>Date :</b> {{ selectedLog.date }}</div>
        <div class="mb-2"><b>Utilisateur :</b> {{ selectedLog.user }}</div>
        <div class="mb-2"><b>Type :</b> {{ selectedLog.type }}</div>
...
const logs = ref([
  { id: 1, date: '2024-06-01 10:12', user: 'admin', type: 'Connexion', object: 'Session', ip: '192.168.1.10', status: 'Succès', session: 'sess-001', description: 'Connexion admin', iso: 'A.12.4.3', raw: {} },
  { id: 2, date: '2024-06-01 10:15', user: 'admin', type: 'Modification droits', object: 'Rôle Auditeur', ip: '192.168.1.10', status: 'Succès', session: 'sess-001', description: 'Ajout droit audit', iso: 'A.12.4.3', raw: {} },
  { id: 3, date: '2024-06-01 10:18', user: 'auditeur', type: 'Tentative accès', object: 'Module Sécurité', ip: '192.168.1.22', status: 'Erreur', session: 'sess-002', description: 'Accès refusé', iso: 'A.12.4.4', raw: {} },
  { id: 4, date: '2024-06-01 10:20', user: 'admin', type: 'Déconnexion', object: 'Session', ip: '192.168.1.10', status: 'Succès', session: 'sess-003', description: 'Déconnexion réussie', iso: 'A.12.4.3', raw: {} },
  { id: 5, date: '2024-06-01 10:22', user: 'auditeur', type: 'Export CSV', object: 'Export', ip: '192.168.1.22', status: 'Succès', session: 'sess-002', description: 'Export des logs', iso: 'A.12.4.4', raw: {} },
])
...
const filters = ref({ keyword: '', user: '', type: '', ip: '', status: '', session: '' })
const hasActiveFilters = computed(() => Object.values(filters.value).some(v => v))
function resetFilters() { filters.value = { keyword: '', user: '', type: '', ip: '', status: '', session: '' } }
...
function printSelected() {
  ...
  if (!win) return
  win.document.write('<html><head><title>Impression logs</title><style>body{font-family:sans-serif;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ccc;padding:6px;}th{background:#f9eaea;}</style></head><body>')
  win.document.write('<h2>Logs sélectionnés</h2>')
